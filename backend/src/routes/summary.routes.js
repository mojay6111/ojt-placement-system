...
router.get(
  ...
  async (req, res) => {
    try {
      ...
      // Sort: category first, then course, then level
      ...
      const totals = {
        PLACED_AND_REPORTED: rows.reduce(
          (s, r) => s + r.PLACED_AND_REPORTED,
          0,
        ),
        PLACED_NOT_REPORTED: rows.reduce(
          (s, r) => s + r.PLACED_NOT_REPORTED,
          0,
        ),
